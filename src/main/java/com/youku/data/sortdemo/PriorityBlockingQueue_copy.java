 ...
 */
public class PriorityBlockingQueue_copy<E> extends AbstractQueue<E> implements
		BlockingQueue<E>, java.io.Serializable {
	private static final long serialVersionUID = 5595510919245408276L;

	/*
	 * The implementation uses an array-based binary heap, with public
	 * operations protected with a single lock. However, allocation during
	 * resizing uses a simple spinlock (used only while not holding main lock)
	 * in order to allow takes to operate concurrently with allocation. This
	 * avoids repeated postponement of waiting consumers and consequent element
	 * build-up. The need to back away from lock during allocation makes it
	 * impossible to simply wrap delegated java.util.PriorityQueue operations
	 * within a lock, as was done in a previous version of this class. To
	 * maintain interoperability, a plain PriorityQueue is still used during
	...
	/**
	 * Creates a {@code PriorityBlockingQueue} with the default initial capacity
	 * (11) that orders its elements according to their {@linkplain Comparable
	 * natural ordering}.
	 */
	public PriorityBlockingQueue_copy() {
	 ...
	 */
	public PriorityBlockingQueue_copy(int initialCapacity) {
	...
	/**
	 * Creates a {@code PriorityBlockingQueue} containing the elements in the
	 * specified collection. If the specified collection is a {@link SortedSet}
	 * or a {@link PriorityQueue}, this priority queue will be ordered according
	 * to the same ordering. Otherwise, this priority queue will be ordered
	 ...
	 */
	public PriorityBlockingQueue_copy(Collection<? extends E> c) {
		this.lock = new ReentrantLock();
		this.notEmpty = lock.newCondition();
		...
		if (c instanceof SortedSet<?>) {
			...
			heapify = false;
		} else if (c instanceof PriorityBlockingQueue_copy<?>) {
			PriorityBlockingQueue_copy<? extends E> pq = (PriorityBlockingQueue_copy<? extends E>) c;
			this.comparator = (Comparator<? super E>) pq.comparator();
			screen = false;
			if (pq.getClass() == PriorityBlockingQueue_copy.class) // exact match
				heapify = false;
		}
		Object[] a = c.toArray();
		int n = a.length;
	...
	/**
	 * Tries to grow array to accommodate at least one more element (but
	 * normally expand by about 50%), giving up (allowing retry) on contention
	...
	private void tryGrow(Object[] array, int oldCap) {
		...
		// 0, 1)
			...
			try {
				int newCap = oldCap + ((oldCap < 64) ? (oldCap + 2) : // grow
																		// faster
																		// if
																		// small
						(oldCap >> 1));
	 ...
	 * otherwise identical. (Similarly for siftDown.) These methods are static,
	 * with heap state as arguments, to simplify use in light of possible
	 ...
	 *         {@code null} if this queue uses the natural ordering of its
	 *         elements
	 */
	 ...
	 * {@code PriorityBlockingQueue} is not capacity constrained.
	 * 
	 ...
	 *
	 * For compatibility with previous version of this class, elements are first
	 * copied to a java.util.PriorityQueue, which is then serialized.
	 */
